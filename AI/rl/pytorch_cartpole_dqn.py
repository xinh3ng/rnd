...
class DQN(nn.Module):
    def __init__(self, h, w, outputs):
        ...
        # Number of Linear input connections depends on output of conv2d layers and therefore the input image size
...
def main(num_episodes: int = 100):
    ...
    print(f"Got no. of actions from gym action space: {n_actions}")
    ...
    for i_episode in range(num_episodes):
        ...
        print(f"Episode: {i_episode} / {num_episodes} has started ...")
        env.reset()
        ...
        for t in count():
            ...
            action = select_action(state=state, policy_net=policy_net, n_actions=n_actions)
            ...
            optimize_model(policy_net=policy_net, target_net=target_net, memory=memory, optimizer=optimizer)
            if done:
        ...
        if i_episode % TARGET_UPDATE == 0:
            target_net.load_state_dict(policy_net.state_dict())
        print(f"Successfully completed episode: {i_episode} / {num_episodes}.")
