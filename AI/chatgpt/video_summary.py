...
def main(
    video_filepath: str = "How it Happened - The 2008 Financial Crisis Crash Course Economics 12.mp4",
    verbose: int = 1,
):
    ...
    logger.info("Converting mp4 to mp3 ...")
    video = E.VideoFileClip(video_filepath)
    video.audio.write_audiofile(audio_filepath)

    if 0 == 1:
        logger.info("Using whisper-1 to get the transcription as text ...")
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Can you kindly summarize the following text in about 20 words: {text}"},
        ],
    )
