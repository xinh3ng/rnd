...
def split_sequences(group_data, n_steps_in, n_steps_out, x_cols, y_cols, additional_columns, step=1, lag_fns=[]):
    ...
    for i in steps:
        ...
        seq_x, seq_y = (
            group_data.iloc[i:end_ix, :][x_cols].values,
            group_data.iloc[end_ix:out_end_ix, :][y_cols + [f"lag_{i}" for i in range(len(lag_fns))]].values,
        )
        for col in additional_columns:
            additional_col_map[col].append(group_data.iloc[end_ix][col])
        X.append(seq_x)
...
def mp_apply(df, func, key_column, workers: int = 6):
...
def sequence_builder(
    ...
    if diff:
        ...
        # first entry in y_cols must be the target variable
        y_col == y_cols[0]
        sequence_fn = partial(
            split_sequences,
            n_steps_in=n_steps_in,
        ...
        sequence_data = mp_apply(
            data[list(set([key_column] + x_cols + y_cols + additional_columns))], sequence_fn, key_column
        )

    sequence_data = pd.DataFrame(sequence_data, columns=["result"])
    s = (
        sequence_data.apply(lambda x: pd.Series(zip(*[col for col in x["result"]])), axis=1)
        .stack()
    ...
    if diff:
        sequence_data[
            ...
            + sorted(set([key_column] + additional_columns + ["x_base", "y_base", "mean_traffic"]))
        ] = pd.DataFrame(sequence_data.result.values.tolist(), index=sequence_data.index)
    else:
...
if __name__ == "__main__":
    ...
    sequence_data.to_pickle(outfile)
