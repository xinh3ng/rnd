...
def add_sentiment_score(data, verbose=1):
    ...
    data["Usefulness"] = (data["VotesHelpful"] / data["VotesTotal"]).apply(
        lambda x: "useful" if x > 0.8 else "useless")
    if verbose >= 1:
        logger.info("Successfully gen categorical sentiment scores. Showing 10 random examples:\n%s" %\
...
def gen_x_tfidf(train_data, test_data, verbose=1):
    ...
    if verbose >= 1:
        logger.info("Showing first and last 5 word features:\n%s\n%s\n" % (str(word_features[:5]),
                    str(word_features[-5:])))
...
""")
    ...
    for name, model in {"bernoullinb": BernoulliNB(),
        ...
        perf = metrics.classification_report(y_val, predictions,
                                             target_names=["positive", "negative"])
