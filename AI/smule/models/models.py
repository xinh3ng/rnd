...
class MFALSOperator(object):
    def __init__(self, params, user_col=DC.user_col,
    ...
    def fit_model(self, data, verbose=0):
        ...
        als = ALS(maxIter=self.params['max_iter'], regParam=self.params['reg_param'],
                  alpha=self.params['alpha'], rank=self.params['rank'],
                  userCol=self.user_col, itemCol=self.item_col, ratingCol=self.rating_col,
                  implicitPrefs=True, nonnegative=False, checkpointInterval=10,
    ...
    def evaluate_model(self, model_fit, data, metric='rmse', verbose=0):
        evaluator = RegressionEvaluator(metricName=metric, labelCol=self.rating_col,
...
def train_validate_mf_als(train_data, val_data, params):
    ...
    perf_row = pd.DataFrame([{'rmse': perf_score,
