...
def random_split_by_user(data, val_size, user_col="user_id", item_col="item_id", rating_col="count", seed=None):
    ...
    train_df = (
        ...
        .apply(lambda x: x[[item_col, rating_col]].sample(frac=train_size, replace=False, random_state=seed))
        .reset_index()
    )
    ...
    logger.info("Fraction of validation data is: %.2f percent" % (100.0 * len(val_df) / len(data)))
    return train_df, val_df


def ensure_sparsity(data, sparsity, user_col=DC.user_col, item_col=DC.item_col, rating_col=DC.rating_col):
    num_non_zeros = sum(data[rating_col] > 0)
    num_zeros = int(sparsity / (1.0 - sparsity) * num_non_zeros)  # number of zeros needed
    ...
    logger.info(
        "After ensure_sparsity, the sparsity is: %.4f pct" % (100.0 * (1.0 - newdata[rating_col].sum() / len(newdata)))
    )
    return newdata
