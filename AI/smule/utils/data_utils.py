...
def load_process_raw(filepath):
    ...
    """
    raw = pd.read_csv(filepath, sep="\t", header=None, skipinitialspace=True)
    ...
    music_items = pd.DataFrame.from_dict({"item": unique_items, "item_id": range(len(unique_items))})
    ...
    logger.info(
        "No. of users: %d, No. of items: %d" % (len(pd.unique(raw[DC.user_col])), len(pd.unique(raw[DC.item_col])))
    )
...
def gen_mf_data(data, user_col=DC.user_col, item_col=DC.item_col, verbose=0):
    ...
    if verbose >= 1:
        ...
        logger.info("Summary stats on newdata:\n%s" % newdata.describe().to_string())
    return newdata


def summary_stats(data, src_name=""):
    logger.info(
        "%s: no. of users who repeat one item twice on more: %d"
        % (src_name, sum(data[[DC.user_col, DC.item_col]].duplicated()))
    )
    logger.info(
        "%s: summary stats on items per user:\n%s"
        % (src_name, data.groupby([DC.user_col]).size().describe().to_string())
    )
    logger.info(
        "%s: summary stats on users per item:\n%s"
        % (src_name, data.groupby([DC.item_col]).size().describe().to_string())
    )
    return


def gen_train_val_data(
    data, total_size, val_size, user_col=DC.user_col, item_col=DC.item_col, rating_col=DC.rating_col, sparsity=0.99
):
    ...
    data = data.copy()
    data, _ = train_test_split(data, test_size=1.0 - total_size)  # Throw away some to speed up training
    ...
    if val_size > 0:
        ...
        val_data = add_zero_observations(val_data, user_col, item_col, rating_col)  # Add 0 observations
...
def add_zero_observations(data, user_col, item_col, rating_col):
    ...
    newdata = pd.DataFrame(list(product(set(data[user_col]), set(data[item_col]))), columns=[user_col, item_col])
    newdata = newdata.merge(data[[user_col, item_col, rating_col]], how="left", on=[user_col, item_col]).fillna(0.0)
    logger.info(
        "After add_zero_observations, sparsity of data: %.4f pct"
        % (100.0 * (1.0 - newdata[rating_col].sum() / len(newdata)))
    )
    return newdata
