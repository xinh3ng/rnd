...
def backtest_model(train_data, estimator,
                   train_size, n_ahead,
                   verbose=0):

    ts_splitter = TimeSeriesSplitter(train_size=train_size, n_ahead=n_ahead)
    perf_df = ts_cross_validate(train_data, estimator, ts_splitter,
                                perf_score_fn=gen_perf_scores,
                                verbose=1)
...
def main(data_dir, train_size=100, n_ahead=2):
    ...
    cv_report = backtest_model(train_data, estimator, train_size,
                               n_ahead=n_ahead,
                               verbose=1)
    return cv_report
