...
def add_sentiment_score(data, verbose=0):
    ...
    data['Usefulness'] = (data["VotesHelpful"] / data["VotesTotal"]).apply(
        lambda n: 'useful' if n > 0.8 else 'useless')
    if verbose >= 1:
        logger.info('Successfully added sentiment scores:\n%s' %\
                    data.sample(5).to_string(line_width=line_width))
    return data
...
def show_user_summary_stats(data):
    ...
    logger.info('Showing top 20 users in terms of total reviews:\n%s' %\
                data.head(20).to_string(line_width=line_width))


def gen_x_tfidf(train_data, test_data, verbose=0):
    if verbose >= 1:
    ...
    if verbose >= 1:
        ...
        logger.info('Shape of train and test features: %s, %s' %\
                    (str(x_train_tfidf.shape), str(x_test_tfidf.shape)))
    return x_train_tfidf, x_test_tfidf, word_features


def main(val_pct=0.1):
    con = sqlite3.connect('/home/{}/data/amazon-fine-good-reviews/database.sqlite'.format(os.environ['USER']))
    raw = pd.read_sql_query(con=con,
                            sql="""
SELECT * 
FROM Reviews 
ORDER BY RANDOM() 
...
""")
    ...
    y_train, y_val = train_data["Sentiment"], val_data["Sentiment"]
    ...
    for name, model in {'bernoullinb': BernoulliNB(),
                        'logit': LogisticRegression(C=1e5)}.items():
    ...
    feature_coefs = pd.DataFrame(
        data=list(zip(word_features_train, model.coef_[0])),
        columns=['feature', 'coef']).sort_values(by='coef')
    logger.info('Showing top 10 negative words:\n%s' % \
                feature_coefs.head(10).to_string(line_width=line_width))
    logger.info('Showing top 10 positive words:\n%s' % \
                feature_coefs.tail(10).to_string(line_width=line_width))
    return
