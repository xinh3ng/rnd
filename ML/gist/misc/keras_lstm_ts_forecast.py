# |/usr/bin/env python
"""Keras, LSTM, Time series forecast on flights
...
def split_dataset(dataset: numpy.ndarray, train_size, look_back) -> (numpy.ndarray, numpy.ndarray):
    ...
    """
    assert train_size > look_back, "train_size must be lager than look_back"
...
def build_model(look_back: int, batch_size: int = 1) -> Sequential:
    ...
    model = Sequential()
    model.add(
        LSTM(64, activation="relu", batch_input_shape=(batch_size, look_back, 1), stateful=True, return_sequences=False)
    )
...
def plot_data(
    ...
    """
    plt.plot(dataset)
    plt.plot([None for _ in range(look_back)] + [x for x in train_predict])
    plt.plot([None for _ in range(look_back)] + [None for _ in train_predict] + [x for x in test_predict])
    plt.plot(
